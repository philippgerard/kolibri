...
class KolibriController extends Controller
{
    ...
    public function loginAction()
    {
        ...
        if ($_POST) {
            ...
            $password = $filter->sanitize($_POST['password'], ['trim']);
    ...
    public function saveAction()
    {
        ...
        if (!$form->isValid($_POST)) {
            ...
            $filter = new \Phalcon\Filter();
            $page = new Pages();
            $page->content = $filter->sanitize($_POST['content'], ['trim']);
            $page->title = $filter->sanitize($_POST['title'], ['string', 'trim']);
            $page->create();
    ...
    public function exportAction()
    {
        $page = Pages::findFirst(
            [
                'conditions' => 'id = :id:',
                'bind'       => ['id' => $this->dispatcher->getParam('id')],
            ]
        );
        if (!$page) {
            return $this->dispatcher->forward(['action' => 'error404']);
    ...
    public function reimportAction()
    {
        ...
        $filter = new \Phalcon\Filter();
        $page->content = $filter->sanitize($oldRevision->content, ['trim']);
        $page->update();
        $curVersion = Versions::maximum(
            [
                'column'     => 'version',
